...
{
    internal class ValueLessThanOrEqualToken<TVal> : ValueToken<TVal>
    {
        // ReSharper disable once InconsistentNaming
        public const string NAME = "GenericValueLessThanOrEqualToken";

        private readonly TVal _valueToCompare;
        private readonly Type _typeOfValueToCompare;

        public ValueLessThanOrEqualToken(VerifiableMemberContract contract, TVal valueToCompare) : base(contract)
        {
            _valueToCompare = valueToCompare;
            _typeOfValueToCompare = typeof(TVal);
        }

        public override CorrectValueOps Ops => CorrectValueOps.LessThanOrEqual_T1;
